...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=16, in_chans=3,
                 ...
                 mae_mask_concat=False):
        super().__init__()
        self.mae_mask_concat = mae_mask_concat
    ...
    def forward_decoder_return_feature(self, x, mask, mask_small, output_block=6):
        # embed tokens
        x = self.decoder_embed(x)  # N,L,D
        N, L, D = x.shape  # batch, length, dim
        if mask is not None:
        ...
        for i, blk in enumerate(self.decoder_blocks):
            if i == output_block:
                break
            if self.mask_decoder and mask_small is not None:
                x, _ = blk(x, mask_small)
            else:
                x, _ = blk(x)
        # only output mask tokens
    ...
    def forward_return_feature(self, imgs, mask, decoder_layer=6):
        """
        ...
        pred = self.forward_decoder_return_feature(latent, new_mask, mask_small, output_block=decoder_layer)  # [N, L, D]
        if self.mae_mask_concat:
            pred = torch.cat([pred, new_mask.unsqueeze(-1)], dim=-1)
...
class MaskedAutoencoderViTFinetune(MaskedAutoencoderViT):
    ...
    def __init__(self, img_size=256, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, random_mask=False, mask_decoder=False, mae_mask_concat=False):
    ...
    def forward_decoder_return_feature(self, x, mask, imgs, irr_mask, mask_small, output_block=6):
        # embed tokens
        x = self.decoder_embed(x)  # N,L,D
    ...
    def forward_decoder_return_feature_and_image(self, x, mask, imgs, irr_mask, mask_small, output_block=6):
        # embed tokens
        x = self.decoder_embed(x)  # N,L,D
    ...
    def forward_return_feature(self, imgs, mask, decoder_layer=6):
        ...
        """
        latent, new_mask, mask_small = self.forward_encoder_with_mask(imgs, mask)
        pred, mae_image = self.forward_decoder_return_feature_and_image(latent, new_mask, imgs, mask, mask_small, output_block=decoder_layer)  # [N, L, D]
        if self.mae_mask_concat:
            pred = torch.cat([pred, new_mask.unsqueeze(-1)], dim=-1)
        # N, L, D = pred.shape  # batch, length, dim
        # s = int(np.sqrt(L))
        # pred = pred.reshape(N, s, s, D).permute(0, 3, 1, 2)
        # return pred, mae_image
        # return pred, scores
        return pred
